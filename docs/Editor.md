...
var editor = new Ext.Editor({ updateEl: true, // update the innerHTML of the bound element when editing completes field: { xtype: 'textfield' } }); var el = Ext.get('my-text'); // The element to 'edit' editor.startEdit(el); // The value of the field will be taken as the innerHTML of the element.
...
The position to align to (see Ext.util.Positionable.alignTo for more details). Defaults to: `'c-c?'`
...
True to complete the editing process if in edit mode when the field is blurred. ...
...
The base CSS class to apply to this component's element. This will also be prepended to elements within this component like Panel's body will get a class `x-panel-body`. This means that if you create a subclass of Panel, and you want it to get all the Panels styling for the element and the body, you leave the `baseCls` `x-panel` and use `componentCls` to add specific styling for this component. Defaults to: `Ext.baseCSSPrefix + 'editor'` Overrides: Ext.container.AbstractContainer.baseCls
...
True to constrain the editor to the viewport Defaults to: `false` Overrides: Ext.util.Floating.constrain
...
Do not participate in the ZIndexManager's focus switching operations. When an editor is hidden, the ZIndexManager will not automatically activate the last visible floater on the stack. Specifies whether the floated component should be automatically focused when it is brought to the front. Defaults to: `false` Overrides: Ext.util.Floating.focusOnToFront
...
private overrides true to hide the component. ...

private overrides `true` to hide the component. Defaults to: `true` Available since: 2.3.0 Overrides: Ext.AbstractComponent.hidden
...
True to skip the edit completion process (no save, no events fired) if the user completes an edit and the value has n...
...
The offsets to use when aligning (see Ext.util.Positionable.alignTo for more details. Defaults to: `[0, 0]`
...
An element to render to. Defaults to: `document.body`
...
"sides" for sides/bottom only, "frame" for 4-way shadow, and "drop" for bottom-right shadow. Defaults to: `'frame'` Overrides: Ext.util.Floating.shadow
...
The targetCls is a CSS class that the layout needs added to the targetEl. The targetEl is where the container's children are rendered and is usually just the main el. Some containers (e.g. panels) use a body instead. In general, if a class overrides getTargetEl it will also need to override this method. This is necessary to avoid a post-render step to add the targetCls.
...
**Parameters:** remainVisible : Boolean (optional)Override the default behavior and keep the editor visible after cancel Defaults to: `false`
...
**Parameters:** remainVisible : Boolean (optional)Override the default behavior and keep the editor visible after edit Defaults to: `false`
...
**Parameters:** remainVisible : Object
...
private Possibly animates down to a target element. ...

private Possibly animates down to a target element. Allows addition of behavior to the hide operation. After calling the superclass’s onHide, the Component will be hidden. Gets passed the same parameters as hide. This is a template method. a hook into the functionality of this class. Feel free to override it in child classes.
...
private Allows addition of behavior to the show operation. ...
...
**Parameters:** autoSize : Boolean (optional)True to size the field to the dimensions of the bound element.
...
**Parameters:** value : ObjectAny valid value supported by the underlying field
...
**Parameters:** el : String/HTMLElement/Ext.ElementThe element to edit
