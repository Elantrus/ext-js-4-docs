...
Defaults to: `Ext.baseCSSPrefix + 'container'`Overrides: Ext.AbstractComponent.baseCls
...
setSize method.
Defaults to: `'body'`Overrides: Ext.AbstractComponent.componentLayout
...
Defaults to: `'tool'`        Available since: 2.3.0
...
Defaults to: `['<div id="{id}-body" class="{headerCls}-body {baseCls}-body {bodyCls} {bodyTargetCls}', '<tpl for="uiCls"> {parent.baseCls}-body-{parent.ui}-{.}</tpl>"', '<tpl if="bodyStyle"> style="{bodyStyle}"</tpl>>', '{%this.renderContainer(out,values)%}', '</div>']`Overrides: Ext.container.AbstractContainer.renderTpl
...
a class for styling that is shared between panel and window headers
Defaults to: `Ext.baseCSSPrefix + 'header'`
...
Gets the tools for this header.
ReturnsExt.panel.Tool[]The tools
...
Parametersicon : StringThe new icon path
...
Parameterscls : StringThe new CSS class name
...
Sets the title of the header.
Parameterstitle : StringThe title to be set
...
Parametersthis : Ext.panel.Header
